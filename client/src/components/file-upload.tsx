    ...
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center text-center">
          ...
          <p className="mb-1.5 text-sm font-medium">{title || "Upload File"}</p>
          <p className="text-muted-foreground text-xs">
            {description || `Drag & drop or click to browse (max. ${formatBytes(maxSize)})`}
          </p>
          {maxSizeDescription && (
            <p className="text-muted-foreground text-xs mt-2 [&>span]:text-theme-2"
              dangerouslySetInnerHTML={{ __html: maxSizeDescription }}
            />
          )}
        </div>
      </div>
