...
import { DocumentReview, CommentItem } from "@/types";
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm flex items- start justify-between">
          ...
          <div className="flex gap-2 items-center">
                    ...
                    {/* <Tabs.Content value="comments" className="mt-4">
                      {review.comment.length === 0 ? (
                        ...
                        <ul className="space-y-3">
                          {review.comments.map((c: CommentItem) => (
                            <li key={c.id} className="p-3 bg-slate-50 rounded">
                              <div className="text-xs text-slate-500">{c.author} · {new Date(c.date).toLocaleString()}</div>
            ...
            {/* Bouton Statut : lecture seule */}
            {/* <button
              className={`px-3 py-1 rounded-md text-sm ${statusClasses[review.status]}`}
              aria-label={`Statut: ${review.status}`}
              title={`Statut: ${review.status}`}
            >
